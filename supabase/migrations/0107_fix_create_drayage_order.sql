-- Dock2Door — Fix create_drayage_order overload ambiguity + stale schema cache
-- Symptom: "Could not find the function public.create_drayage_order(...) in the schema cache"
--
-- Root cause: migrations 0100 and 0103 each defined a create_drayage_order with a
-- DIFFERENT argument list (28 args without target company, 29 args with it). PostgREST
-- matches RPC calls by the EXACT set of named arguments provided. With two overloads and
-- none of the params carrying defaults, a call that doesn't line up with either signature
-- fails to resolve.
--
-- Fix: drop BOTH old signatures and create ONE canonical function whose optional trailing
-- args have DEFAULTs, so calls resolve whether or not p_is_oversized / p_target company are
-- supplied. Then force a schema-cache reload.

-- =========================================================================
-- 1) DROP the old overloads (both signatures)
-- =========================================================================
drop function if exists public.create_drayage_order(
...
-- =========================================================================
-- 2) CANONICAL function — optional trailing args carry defaults
-- =========================================================================
...
grant execute on function public.create_drayage_order(
  ...
  text, text, numeric, numeric, date, text, boolean, date, uuid, text, boolean, uuid
) to authenticated;

-- =========================================================================
-- 3) Force PostgREST to reload its schema cache so the new signature is seen
-- =========================================================================
