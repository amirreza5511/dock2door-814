-- Migration 0089: Fix worker_apply_shift enum bug.
-- The fill-check compared shift_assignments.status against 'HoursConfirmed',
-- which is NOT a member of the assignment_status enum
-- ('Scheduled','InProgress','Completed','NoShow','Cancelled','Disputed' — 0001:88).
-- Postgres raised "invalid input value for enum assignment_status: HoursConfirmed"
-- whenever a worker tapped Apply, blocking all applications. Drop the bogus value.
...
begin
  ...
  -- Only valid assignment_status values here — 'HoursConfirmed' does not exist.
