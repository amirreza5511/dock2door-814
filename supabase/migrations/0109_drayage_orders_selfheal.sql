-- Dock2Door — Self-healing drayage order creation
--
-- Symptom (live DB): submitting a container order fails with
--   [42703 | column "target_drayage_company_id" of relation "drayage_orders" does not exist]
--
-- Root cause: earlier migrations that ADD COLUMNS (0103 target company,
-- 0108 handling mode) were not all applied on the live database, while the
-- create_drayage_order function DID get recreated referencing those columns.
-- Recreating the function alone can never fix a missing column.
--
-- This migration is fully self-contained and idempotent: it guarantees EVERY
-- column the function writes to exists BEFORE (re)creating the function, drops
-- all stale overloads, then refreshes the PostgREST schema cache.

-- =========================================================================
-- 0) Enums used below (create if missing)
-- =========================================================================
do $$ begin
...
-- =========================================================================
-- 1) Guarantee every column the function needs exists on drayage_orders
-- =========================================================================
...
alter table public.drayage_orders
  add column if not exists is_prepull boolean not null default false;

alter table public.drayage_orders
  add column if not exists prepull_pickup_date date;

alter table public.drayage_orders
  add column if not exists prepull_yard_terminal_id uuid references public.terminals(id) on delete set null;
