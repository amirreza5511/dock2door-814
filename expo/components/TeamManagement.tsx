...
/** Generic office/company roles available to every company type. */
export const GENERIC_COMPANY_ROLES: CompanyRole[] = ['Owner', 'Manager', 'Supervisor', 'Staff', 'ReadOnly'];

/** Full warehouse operational roles (receiving, picking, packing, shipping, etc.). */
export const WAREHOUSE_COMPANY_ROLES: CompanyRole[] = [
  'Owner', 'Manager', 'Supervisor', 'Receiver', 'Picker', 'Packer',
  'ShippingClerk', 'InventoryClerk', 'DockStaff', 'ReadOnly',
];

interface TeamManagementProps {
  /** Screen title. Defaults to "Team". */
  title?: string;
  /** Short helper subtitle shown under the title. */
  subtitle?: string;
  /** Which company roles are offered when inviting / editing. Defaults to generic roles. */
  roleOptions?: CompanyRole[];
  /** Default role selected in the invite modal. */
  defaultRole?: CompanyRole;
}

/**
 * Reusable team / staff management surface. Works for any company because it is
 * backed by the generic `company_users` table + `company_*` RPCs. Owners and
 * Managers can invite members, change roles, suspend, reactivate and remove.
 */
export default function TeamManagement({
  ...
  const inviteRoles = useMemo<CompanyRole[]>(() => roleOptions.filter((r) => r !== 'Owner'), [roleOptions]);
  const initialRole: CompanyRole = defaultRole && inviteRoles.includes(defaultRole) ? defaultRole : (inviteRoles[0] ?? 'Staff');
  ...
  const [editRole, setEditRole] = useState<CompanyRole>(initialRole);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setEmail('');
      setAddOpen(false);
      Alert.alert('Member added', `${clean} added as ${ROLE_LABEL[role]}.`);
    } catch (e) {
  ...
  const filterChips: (CompanyRole | 'All')[] = ['All', ...roleOptions];
      ...
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        <View style={styles.headerRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sub}>{subtitle ?? `${members.length} member${members.length === 1 ? '' : 's'} · ${activeCompany?.companyName ?? ''}`}</Text>
          </View>
          {canManage && (
            <TouchableOpacity onPress={() => { setRole(initialRole); setAddOpen(true); }} style={styles.addHeaderBtn} testID="open-add-staff">
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 100 }]}>
        {!canManage && (
          <Card style={styles.notice}>
            <Text style={styles.noticeText}>You have view-only access to the team. Only Owner or Manager can invite or change roles.</Text>
          </Card>
        )}
