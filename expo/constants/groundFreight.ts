import { Truck, Boxes, Layers } from 'lucide-react-native';
import type { LucideIcon } from 'lucide-react-native';
import type { FreightMode } from '@/constants/globalFreight';

/**
 * LTL & FTL Quotes (ground freight world) — shared constants for the customer
 * post flow, provider board and the instant ballpark estimate. Reuses the
 * existing freight quote/offer engine; ground load types map onto the engine's
 * freight_mode values (LTL/FTL → truck/fcl, LCL → lcl).
 */
...
export const LOAD_TYPES: LoadTypeDef[] = [
  { value: 'ltl', label: 'LTL — part load', short: 'LTL', sublabel: 'A few pallets, share the truck', icon: Layers, freightMode: 'truck' },
  { value: 'ftl', label: 'FTL — full truck', short: 'FTL', sublabel: 'A whole truck to yourself', icon: Truck, freightMode: 'fcl' },
  { value: 'lcl', label: 'LCL — shared container', short: 'LCL', sublabel: 'Share container space by volume', icon: Boxes, freightMode: 'lcl' },
];
