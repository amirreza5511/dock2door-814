...
import { ArrowLeft, Award, Star, Shield, MapPin, DollarSign, CheckCircle, Camera } from 'lucide-react-native';
...
async function fetchWorkerById(userId: string) {
  const [profileRes, certsRes, photosRes, reviewsRes] = await Promise.all([
    ...
    supabase
      ...
      .eq('status', 'Approved'),
    // Moderation-approved public/company photos only
    supabase
      .from('work_photos')
...
export default function WorkerPublicProfileById() {
  ...
  const { profile, certs, photos, reviews } = profileQuery.data ?? {
    profile: null,
    certs: [],
    photos: [],
    reviews: [],
  ...
  return (
    <View style={[styles.root, { backgroundColor: C.bg }]}>
      ...
      >
        ...
        {/* Moderation-filtered work photos */}
        {photos.length > 0 && (
          <View style={styles.section}>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: '700' as const, color: C.text, flex: 1, textAlign: 'center' },
  scroll: { padding: 16 },
  ...
  avatarText: { fontSize: 34, fontWeight: '800' as const, color: C.accent },
  displayName: { fontSize: 22, fontWeight: '800' as const, color: C.text, marginBottom: 4 },
  tagline: { fontSize: 13, color: C.accent, fontWeight: '600' as const, marginBottom: 8 },
