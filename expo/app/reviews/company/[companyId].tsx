...
import { ArrowLeft, Star, Building2, MessageSquare } from 'lucide-react-native';
...
export default function CompanyReviewsScreen() {
  ...
  const summaryQuery = trpc.reviews.companySummary.useQuery({ companyId: String(companyId) }, { enabled: Boolean(companyId) });
  const listQuery = trpc.reviews.listForCompany.useQuery({ companyId: String(companyId) }, { enabled: Boolean(companyId) });

  const reviews = useMemo<ReviewRow[]>(() => (listQuery.data ?? []) as ReviewRow[], [listQuery.data]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: C.bg }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.title}>Company Ratings</Text>
      </View>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 80 }]}>
        <Card elevated style={styles.summaryCard}>
          <View style={styles.summaryIcon}><Building2 size={20} color={C.accent} /></View>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.summaryCount}>{summary.count ?? 0} review{Number(summary.count) === 1 ? '' : 's'}</Text>
          </View>
        </Card>

        {reviews.length === 0 ? (
          <EmptyState icon={MessageSquare} title="No reviews yet" description="Completed bookings and jobs will surface public ratings here." />
        ) : reviews.map((r) => (
          <Card key={r.id} style={styles.reviewCard}>
            ...
            <View style={styles.reviewBottom}>
              ...
              <Text style={styles.reviewer}>{r.reviewerCompanyName ?? r.reviewerName ?? 'Verified customer'} · {r.contextKind.replace('_', ' ')}</Text>
            </View>
          </Card>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  summaryIcon: { width: 52, height: 52, borderRadius: 14, backgroundColor: C.accentDim, alignItems: 'center', justifyContent: 'center' },
