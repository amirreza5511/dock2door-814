/**
 * Skills catalog — the single source of truth for the web app. Mirrors
 * `expo/constants/skills.ts` exactly. The `id` values MUST match the Postgres
 * `shift_category` enum labels (see migration 0130).
 */
...
export const ALL_SKILL_IDS: SkillId[] = ALL_SKILLS.map((s) => s.id);
