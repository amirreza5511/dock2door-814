/**
 * Help Center content — web mirror of the mobile app's `expo/constants/help.ts`.
 * Role manuals grouped by product world, plus shared screens and route mapping
 * adapted to the web app's URL structure.
 */
import {
  Building2, Clock, Crown, DoorOpen, PackageOpen, ShieldCheck, Truck,
  UserCog, Users, Warehouse, Wrench, type LucideIcon,
} from "lucide-react";
...
export interface RoleDoc {
  ...
  icon: LucideIcon;
  /** Accent color (hex) used for this role's manual. */
  color: string;
  overview: string;
  screens: ScreenDoc[];
}
...
export const HELP_WORLDS: WorldDoc[] = [
  { key: "freight", title: "Freight & Delivery", blurb: "Post loads, accept them, and deliver — Uber for trucks.", color: GREEN },
  { key: "logistics", title: "Logistics & Warehousing", blurb: "Warehouse space, industrial services and fulfillment.", color: TEAL },
  { key: "labour", title: "Labour", blurb: "Post shifts and find workers fast.", color: PURPLE },
  { key: "admin", title: "Administration", blurb: "Run, price and police the whole platform.", color: RED },
];
...
/** Base web path for each manual role key (web URLs differ from mobile in places). */
...
/** Screen-level overrides where the web route differs from the mobile screen id. */
const WEB_ROUTE_OVERRIDES: Record<string, string> = {
  "shipper:track": "/shipper/loads",
  "trucking-company:messages": "/messages",
  "warehouse-provider:create-listing": "/warehouse/listings/new",
  "warehouse-provider:station-receiving": "/warehouse/stations/receiving",
  "warehouse-provider:station-picking": "/warehouse/stations/picking",
  "warehouse-provider:station-packing": "/warehouse/stations/packing",
  "warehouse-provider:station-shipping": "/warehouse/stations/shipping",
  "warehouse-provider:station-inventory": "/warehouse/stations/inventory",
  "warehouse-provider:station-dock": "/warehouse/stations/dock",
  "worker:browse": "/worker/browse-shifts",
  "worker:my-shifts": "/worker/shifts",
  "admin:audit-logs": "/admin/audit",
  "admin:notifications-health": "/admin/health",
};

/** Resolves the in-app web route for a role + screen so the manual can deep-link to it. */
export function getScreenRoute(roleKey: string, screenId: string): string {
  const override = WEB_ROUTE_OVERRIDES[`${roleKey}:${screenId}`];
  if (override) return override;
  const base = WEB_BASE[roleKey] ?? `/${roleKey}`;
  return screenId === "index" ? base : `${base}/${screenId}`;
}
