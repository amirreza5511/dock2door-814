...
import { Ship, Container, CreditCard } from "lucide-react";
...
/** Guests can post and track container drayage orders like any customer — prepaid, with guest surcharge. */
export default function GuestOrdersPage() {
  const supabase = getBrowserSupabase();
  const q = useQuery({
    queryKey: ["guest", "drayage-orders"],
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Guest orders — prepaid, with guest surcharge.</p>
      </div>
      ...
      <Card className="border-yellow-500/30">
        <CardContent className="flex items-center gap-3 pt-6">
          ...
          <p className="text-sm text-muted-foreground">
            As a guest, every order requires <span className="font-medium text-foreground">prepayment</span> and includes the guest
            surcharge. Pay invoices in <Link href="/guest/billing" className="text-primary underline-offset-2 hover:underline">Billing &amp; prepay</Link>.
          </p>
        </CardContent>
      </Card>
